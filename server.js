...
function defaultTemplateDefs() {
  ...
  ], ['序号', '物资编码', '物资名称', '物资型号', '单位', '入库数量', 'SN/序列号', '备注'], [6, 14, 22, 14, 8, 10, 30, 18]);
  defs.out = sheetInfo('出库单', [
    ['单据编号：{{doc_no}}', '出库日期：{{date}}'],
    ['客户/领用部门：{{party}}', '经办人：{{operator}}'],
  ], ['序号', '物资编码', '物资名称', '物资型号', '单位', '出库数量', 'SN/序列号', '备注'], [6, 14, 22, 14, 8, 10, 30, 18]);
...
function fmtPdfRows(doc, lines) {
  ...
  for (const l of lines) {
    ...
    if ((doc.type === 'in' || doc.type === 'out') && snManaged && sns.length) {
      for (const sn of sns) rows.push({ sku_code: l.sku_code, name: l.name, spec: l.spec, unit: l.unit, qty: 1, sn });
    } else {
      rows.push({ sku_code: l.sku_code, name: l.name, spec: l.spec, unit: l.unit, qty: Math.abs(l.qty), sn: sns.length ? sns.join(' / ') : '' });
    }
  }
  return rows;
}
function renderDocPdf(doc, lines) {
  return new Promise((resolve, reject) => {
    ...
    const rows = fmtPdfRows(doc, lines);
    const chunks = [];
    const dd = new PDFDocument({ size: 'A4', margin: 36, info: { Title: `${doc.doc_no}` } });
    ...
    const label = DOC_META[doc.type].label;
    // 标题
    dd.font('cjk').fontSize(18).fillColor('#111');
    dd.text(label, ML, 34, { width: avail, align: 'center' });
    dd.moveDown(0.6);
    dd.fontSize(9).fillColor('#666').text(`${doc.doc_no}    日期：${dateOf(doc.created_at)}`, ML, 54, { width: avail, align: 'center' });
    // 右上角二维码
    qrPng(payloadEncode(pl), 160).then(buf => {
      dd.image(buf, PW - MR - 74, 36, { width: 72, height: 72 });
      dd.font('cjk').fontSize(7).fillColor('#888').text('扫此码可导入本单', PW - MR - 74, 110, { width: 74, align: 'center' });
      // 信息区
      dd.fontSize(10).fillColor('#111');
      let y = 128;
      const infoLines = [`往来单位：${doc.party || '—'}`, `经办人：${doc.operator || '—'}`, `库位：${doc.location || '—'}`, `备注：${doc.remark || '—'}`];
      dd.font('cjk').text(infoLines.join('    '), ML, y, { width: avail - 90, lineBreak: true, paragraphGap: 3 });
      y += 40;
      // 表头
      const cols = [
        { w: 34, h: '序号' }, { w: 96, h: 'SKU编码' }, { w: 148, h: '名称' }, { w: 78, h: '规格' }, { w: 40, h: '单位' }, { w: 44, h: '数量' }, { w: 0, h: 'SN / 序列号' }
      ];
      ...
      rows.forEach(r => {
        ...
        drawRow([rows.indexOf(r) + 1, r.sku_code, r.name, r.spec || '', r.unit || '', r.qty || '', r.sn || ''], false, rowH);
      });
      // 合计
      dd.font('cjk').fontSize(9).fillColor('#111');
      dd.text(`合计：${rows.length} 行 / ${total} 件`, ML, y2 + 6);
      dd.text(`制单人：${doc.operator || '—'}`, ML, y2 + 20);
      dd.end();
    }).catch(reject);
  });
}
